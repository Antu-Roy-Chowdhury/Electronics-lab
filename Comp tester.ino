...
typedef struct
{
  ...
  unsigned int                RiL;               //Internal pin resistance of A‚ lC in low mode (0.1 Ohms)
  unsigned int                RiH;               //Internal pin resistance of A‚ lC in high mode (0.1 Ohms)
...
typedef struct
{
  ...
  unsigned int                I_CE0;             //Leakage current (in A‚ lA)
} BJT_Type;
typedef struct
{
...
void CheckProbes(byte Probe1, byte Probe2, byte Probe3)
{
  ...
  if (U_Rl > 490)                                // > 700A‚ lA (was 92mV/130A‚ lA)
  {
...
unsigned long Get_hFE_C(byte Type)
{
  ...
  unsigned int                Ri;                //Internal resistance of A‚ lC
  ...
  if (U_R_b < 10)                                //I_b < 14A‚ lA -> Darlington
  {
    if (Type == TYPE_NPN)                        //NPN
    {
...
void CheckBJTorEnhModeMOSFET(byte BJT_Type, unsigned int U_Rl)
{
  ...
  {
    BJT_Level = 2557;                            //Voltage across base resistor (5.44A‚ lA)
  ...
  {
    BJT_Level = 977;                             //Voltage across base resistor (2.1A‚ lA)
  ...
  if (U_R_b > BJT_Level)                         //U_R_b exceeds minimum level of BJT
  {
    ...
    Check.Type = BJT_Type;
    I_CE0 = GetLeakageCurrent();                 //Get leakage current (in A‚ lA)
...
void CheckResistor(void)
{
  ...
  U_Ri_L = ReadU_5ms(Probes.Pin_2);              //Get voltage at internal R of A‚ lC
  ...
  if (U_Rh_L <= 20)
  {
    ...
    R_DDR = Probes.Rl_2;                         //Pull down probe-2 via Rl
    U_Ri_H = ReadU_5ms(Probes.Pin_1);            //Get voltage at internal R of A‚ lC
...
byte LargeCap(Capacitor_Type *Cap)
{
  ...
  if (Flag == 3)
  {
    ...
    Cap->Raw = Raw;
    Cap->Value = Value;                          //Max. 4.3*10^6nF or 100*10^3A‚ lF
  }
  return Flag;
}
byte SmallCap(Capacitor_Type *Cap)
...
byte MeasureInductance(uint32_t *Time, byte Mode)
{
  ...
  {
    Test = (CPU_FREQ / 1000000);                 //Cycles per A‚ ls
    ADC_PORT = Probes.ADC_1;                     //Pull up probe-1 directly
    while (Test > 0)
    {
      Test--;
  ...
  if (Counter > 0)
  {
    ...
    Counter /= (CPU_FREQ / 1000000);             //Divide by frequeny and scale to A‚ ls
  }
...
byte MeasureInductor(Resistor_Type *Resistor)
{
  ...
  if (Test == 3)
  {
    ...
    Value /= 5;                                  // / 5000mV, * 10^3
    Scale = -6;                                  //A‚ lH by default
    Value = Time1;                               //t_stop
    Value *= Factor;                             // * factor (A‚ ls * 10^-3)
...
void ShowDiode(void)
{
  ...
  if (D2 == NULL)                                //Single diode
  {
    ...
    I_leak = GetLeakageCurrent();                //Get current (in A‚ lA)
    ...
    while (Counter <= 5)
    {
      ...
      switch (Test)
      {
        ...
        case 2:                                  //Un-short probes
          ...
          break;
        case 3:                                  //Internal resistance of A‚ lC in pull-down mode
          ...
          RiL_Counter += 3;
          break;
        case 4:                                  //Internal resistance of A‚ lC in pull-up mode
